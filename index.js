...
const LockDownEssentials = require("./HomeHardware");
...
app.get("/pdf/:phone", (req, res) => {
  // this happens when the user clicks on the link in SMS
  const sFrom = req.params.phone;
  if (!oOrders.hasOwnProperty(sFrom)) {
    res.end("order already complete");
  } else {
    var file = fs.createReadStream('./HomeEssentials.pdf');
    var stat = fs.statSync('./HomeEssentials.pdf');
